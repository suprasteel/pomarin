...
struct VertexOutput {
    ...
    [[location(4)]] tangent_normal: vec3<f32>;
...
fn fs_main(in: VertexOutput) -> [[location(0)]] vec4<f32> {
    ...
    // let diffuse_strength = max(dot(in.tangent_normal, light_dir), 0.0);
    let diffuse_strength = max(dot(in.tangent_normal, light_dir), 0.0);
    let diffuse_color = light.color * diffuse_strength;

    // let specular_strength = pow(max(dot(in.tangent_normal, half_dir), 0.0), 2.0);
    let specular_strength = pow(max(dot(in.tangent_normal, half_dir), 0.0), 64.0);
